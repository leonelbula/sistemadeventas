...
class DatosEmpresa{
	...
	public function Registrar() {
		$sql = "INSERT INTO datos_empresa VALUES (NULL, '{$this->getNit()}', '{$this->getNombre()}',"
				...
				. " '{$this->getTelefono()}', '{$this->getCelular()}', '{$this->getEmail()}', '{$this->getPaginaweb()}',"
				. " '{$this->getEslogan()}')";
		$resul = $this->db->query($sql);
		$resp = FALSE;
		if ($resul) {
	...
	public function Actualizar() {
		...
		. "celular='{$this->getCelular()}',paginaweb='{$this->getPaginaweb()}', eslogan='{$this->getEslogan()}' WHERE id = {$this->getId()}";
